...
test('returns a promise', (t) => {
  const value = t.context.client.removeVisibilityTimeout({ ReceiptHandle: 'foobar' });

  t.true(typeof value.then === 'function');
});

test('calls changeMessageVisbility with the message receipt', async (t) => {
  await t.context.client.removeVisibilityTimeout({ ReceiptHandle: 'foobar' });

  t.true(t.context.client.sqs.changeMessageVisibility.calledWith({ ReceiptHandle: 'foobar', VisibilityTimeout: 0 }));
});
